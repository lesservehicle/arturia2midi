...
def decode_project_file(project_file_path, parameters_file_path):
    ...
    for key, value in project_data.items():
        ...
        if track == "project" and tempo is None:
            ...
            if (
                tempo_msb is not None
                and tempo_mdsb is not None
                and tempo_lsb is not None
            ):
                tempo = (
                    tempo_msb
                    + tempo_mdsb.replace("0b", "")
                    + tempo_lsb.replace("0b", "")
                )
        ...
        decoded_project[track][param_id]["desc"] = decoded_key
        decoded_project[track][param_id][pattern_id][multiplier][note_id][
            "value"
        ] = value
    ...
    for pattern, step_info in track_data.items():
        ...
        for step, note_info in step_info.items():
            ...
            for note_id, attrs in note_info.items():
                ...
                start_time = (
                    step * sixteenth_note_duration * 2
                    + pattern_cursor * sixteenth_note_duration * 2
                )
...
if __name__ == "__main__":
    ...
    if args.markdown:
        ...
        for track in ("device", "project", "track1", "track2", "track3", "track4"):

            for param_id, value in decoded_project_data[track].items():
                if track == "device" or (
                    track == "project" and (param_id == "tempo" or param_id == "swing")
                ):
                    continue
                elif "desc" in value:
                    param = {}
